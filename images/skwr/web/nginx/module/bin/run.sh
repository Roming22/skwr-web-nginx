...
clean_servers(){
  for SERVER in $(find "$TMP_CONF_DIR" -mindepth 1 -maxdepth 1 | sed "s:\.[^.]*$::"); do
    unset RM
...
discover_servers(){
  nmap -p 8000 "$IP_RANGE" | grep -E 'Nmap scan report for |8000/tcp ' | grep -B1 'open' | grep ' for ' | awk '{print $5}' | grep -v "\.0\.1" | while read FQDN; do
    export FQDN
